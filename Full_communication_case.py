...
def main():
    ...
    J_opt_full = J_opt_full_traj if np.isscalar(J_opt_full_traj) else J_opt_full_traj[-1]

    J_dis_full = np.zeros((cfg.dzm, 3))
    
    for ii in range(3):
        T_val = cfg.T[ii]
        _, J_dis_full_temp = optimize_F_trajectory_full_large_scale(
            A, B, F_sparse0, mask, cfg.gam, cfg.dzm, T_val, cfg.D, Q, R, 
            cfg.w, cfg.numAgents, cfg.ex_w, cfg.W, X_0)
        J_dis_full[:, ii] = J_dis_full_temp.flatten()
    ...
    p1, = ax.plot(x_axis, J_dis_full[:, 0], 'b-',  linewidth=2.0)
    p2, = ax.plot(x_axis, J_dis_full[:, 1], 'c--', linewidth=2.5)
    p3, = ax.plot(x_axis, J_dis_full[:, 2], 'm-.', linewidth=2.5)
    ...
    # 设置图例
    leg = ax.legend([p1, p2, p3, p4], ['T=3', 'T=20', 'T=60', 'Optimal cost (full info)'],
